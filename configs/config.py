...
# print(project_root)
data_root = './Image'  # path to dataset

cfg = EasyDict()

# COMMON CONFIGS
cfg.CROPTYPE = 'images'
cfg.SEED = random.choice(range(10000))  # 1234, 3407, 42
cfg.NWORK = 4
...
def get_arguments():
    ...
    parser.add_argument('--use_cache', dest='USE_CACHE', help='use preprocessed cache for acceleration',
                        action='store_true')
    # numeric
    ...
    parser.add_argument('--im_scale', dest='im_scale', type=int,
                        default=896)  # original 896, maxvit_t 224,maxvit_b 512,coatnet 384,maxx_l 512
    ...
    parser.add_argument('--exp_name', type=str, required=False, default="WW2020_swinv2_b", help='data config path')
    parser.add_argument('--restore_from', type=str, required=False, default="")
    parser.add_argument('--codalab_pred', type=str, required=False, default="test",
                        help="save predictions for codalab. test for submission, val for validation")
    parser.add_argument('--train_set', type=str, required=False, default="trainval",
                        help="trainval or train: trainval means no validation set needed")
